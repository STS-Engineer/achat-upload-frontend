...
interface MultiSelectAuditorsProps {
  label: string;
  options: Option[];
  value?: string[];
...
const MultiSelectAuditors: React.FC<MultiSelectAuditorsProps> = ({
  label,
  options,
  value,
  defaultSelected = [],
  ...
  onChange?.(newSelected); // send IDs, not emails
};
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    switch (e.key) {
      ...
      case "ArrowDown":
        ...
        break;
      case "ArrowUp":
        if (isOpen) setFocusedIndex(prev => (prev > 0 ? prev - 1 : options.length - 1));
        break;
    }
  };
